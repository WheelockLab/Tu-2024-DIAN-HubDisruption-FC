...
xvarname = 'EYO'
yvarname = 'CCS'#expression(kappa [s])#'PIB' #  expression(kappa [s])#expression(kappa [s])#'PiB'#expression(kappa [s])#
set.seed(10)
gamm_model <-gamm(CCS~
mutation
+ s(EYO, by=mutation)
...
save(divergencepoint_HDI_P,divergencepoint_HDI_S,divergencepoint_CCS_P,divergencepoint_CCS_S,divergencepoint_PIB_P,divergencepoint_PIB_S,file = "divergent_points_all_bootstrapped_simultaneous83.4.Rdata")

ids = which(!is.na(divergencepoint_HDI_P))[1:1000]
...
```{r histogram}
hist(divergencepoint_PIB_S[ids]-divergencepoint_HDI_S[ids],main = "",xlab=cbind("EYO divergence point (PiB-HDI)"))
mean(divergencepoint_PIB_S[ids]-divergencepoint_HDI_S[ids]>0)
```

...
library(lme4)
library(lmerTest)
setwd("/Volumes/wheelock/people/Cindy/DIAN/Final")
HDI = read.csv('./HDI_individual_acrossthreS(z-score).csv')
covariates = read.csv('./regression_covariates.csv')
```
# get data structure
```{r makedf}
df_all=data.frame(covariates$CCS,covariates$PIB_fSUVR_rsf_TOT_CORTMEAN,HDI$thre5,covariates$Sex.M.,covariates$EYO,covariates$Education,covariates$DVARS,covariates$Minutes,covariates$Age,factor(covariates$fam_mutation),factor(covariates$mutation),covariates$MMSE,covariates$MEMUNITS,covariates$WAIS,covariates$WORDDEL,covariates$family)
names(df_all)<-c("CCS","PIB","HDI","Sex_M","EYO","Education","DVARS","Minutes","Age","fam_mutation","mutation","MMSE","MEMUNITS","WAIS","WORDDEL","family")

df_MC<-filter(df_all,mutation==1)
df_NC<-filter(df_all,mutation==0)
...
#lme + gam
xvarname = 'EYO'
yvarname = 'PiB'#expression(kappa [s])
set.seed(10)
gamm_model <-gamm(PIB~
mutation
+ s(EYO, by=mutation)
...
newdata <- with(df_all, data.frame(
  ...
  Sex_M = rep(c("true", "false"), each = 100),
  DVARS = rep(median(DVARS), 200),
  Education = rep(median(Education), 200)  # using median or any other constant value
...
pred_NC_M <- get_splineBasedCI(gam_mod_all,newdata_NC[1:100,])
pred_NC_F <- get_splineBasedCI(gam_mod_all,newdata_NC[101:200,])
pred_NC<-pred_NC_M
...
pred_MC_M <- get_splineBasedCI(gam_mod_all,newdata_MC[1:100,])
pred_MC_F <- get_splineBasedCI(gam_mod_all,newdata_MC[101:200,])
pred_MC<-pred_MC_M
